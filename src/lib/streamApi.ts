...
export async function getStreamLinks(
  ...
  try {
    ...
    if (type === 'tv' && season && episode && infoData.seasons) {
        const targetSeason = infoData.seasons.find((s: any) => s.season === season);
    ...
    console.warn("Public API Scraper gagal atau diblokir CORS. Menggunakan mode Fallback (Iframe).");
    return null;
  }
}
