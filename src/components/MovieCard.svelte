...
>
  ...
  <div class="absolute bottom-0 left-0 right-0 p-4 flex flex-col gap-1.5 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
    ...
    <div class="flex items-center gap-2 text-xs font-medium mt-0.5">
      <span class="bg-white/20 backdrop-blur-md px-2 py-0.5 rounded text-white/90 uppercase text-[10px] tracking-wider">{type === 'tv' ? 'Series' : 'Movie'}</span>
      {#if year}
        <span class="text-white/70 text-[11px]">• {year}</span>
      {/if}
      {#if rating > 0}
        <span class="flex items-center gap-1 text-yellow-500 font-bold drop-shadow-md ml-auto">
